...
const PlaylistCard = ({ playlist, setPlaylists }) => {
  ...
  useEffect(() => {
    if (playlist.videos.length > 0) {
      makeAPICall(`videos?part=snippet&id=${playlist.videos[0]}`).then((data) => setVideoCard(data?.items[0]?.snippet.thumbnails.standard.url));
    }
  }, []);

  const handlePlaylistEdition = () => {
    loggedUser ? navigate(`/editPlaylistPage/${playlist.playlistID}`) : navigate("/login");
  };
  ...
  return (
    <div className={styles.playlistCard}>
      ...
      <div className={styles.btnContainer}>
        {!!videoCard ? (
          <button className={styles.playlistCardBtn} onClick={() => handlePlaylistEdition()}>
            Edit
          </button>
        ) : null}
        <button
          className={styles.playlistCardBtn}
          onClick={(e) => {
            e.stopPropagation();
            setModalShow(true);
          }}
      ...
      <ModalWindow
        key={playlist.playlistName + playlist.playlistID}
        show={modalShow}
